...
# Windows helpers. We only observe the desktop/window position; the app never
# sends mouse/keyboard input to OSRS.
if os.name == 'nt':
...
def find_osrs_window():
    ...
    def callback(hwnd, _):
        ...
        title = buf.value
        lower = title.lower()
        if 'old school runescape' in lower or lower == 'runelite' or 'jagex launcher' in lower:
...
class Navigator:
    def __init__(self, root):
        ...
        self.cfg = load_json(CONFIG, {
            'x': 80, 'y': 80, 'alpha': 0.94, 'clickthrough': False,
    ...
    def build_ui(self):
        ...
        tabs.add(self.quest_tab, text='Quests')
        tabs.add(self.route_tab, text='Route')
        tabs.add(self.map_tab, text='Overlay')
        ...
        footer = tk.Frame(self.root, bg='#20242a')
        footer.pack(fill='x')
        ...
        tk.Button(footer, text='F8 Hide', command=self.toggle_visible, relief='flat',
                  bg='#30353b', fg='white').pack(side='right', padx=4, pady=5)
        tk.Button(footer, text='F9 Click-through', command=self.toggle_clickthrough,
                  relief='flat', bg='#30353b', fg='white').pack(side='right', padx=4, pady=5)

    def build_dashboard(self):
        tk.Label(self.dashboard_tab, text='Progress', bg='#15171a', fg='white',
                 font=('Segoe UI', 18, 'bold')).pack(anchor='w', padx=14, pady=(16, 6))
        self.progress_text = tk.Label(self.dashboard_tab, text='', bg='#15171a', fg='#c8cdd3',
                                      justify='left', font=('Segoe UI', 10))
        self.progress_text.pack(anchor='w', padx=14, pady=6)

        card = tk.Frame(self.dashboard_tab, bg='#20242a')
        card.pack(fill='x', padx=14, pady=12)
        self.next_text = tk.Label(card, text='', bg='#20242a', fg='white', justify='left',
                                  anchor='w', font=('Segoe UI', 11))
        self.next_text.pack(fill='x', padx=12, pady=12)

        tk.Button(self.dashboard_tab, text='Attach overlay to OSRS', command=self.attach_overlay,
                  bg='#3f6685', fg='white', relief='flat').pack(anchor='w', padx=14, pady=5)
        tk.Button(self.dashboard_tab, text='Recalculate recommended quest', command=self.recommend_quest,
                  bg='#30353b', fg='white', relief='flat').pack(anchor='w', padx=14, pady=5)

    def build_quests(self):
        top = tk.Frame(self.quest_tab, bg='#15171a'); top.pack(fill='x', padx=8, pady=8)
        self.search = tk.StringVar()
        ent = tk.Entry(top, textvariable=self.search, bg='#282d33', fg='white',
                       insertbackground='white', relief='flat')
        ent.pack(fill='x', ipady=7)
        ent.bind('<KeyRelease>', lambda e: self.refresh_quest_list())
        body = tk.Frame(self.quest_tab, bg='#15171a'); body.pack(fill='both', expand=True, padx=8)
        self.listbox = tk.Listbox(body, bg='#20242a', fg='white', selectbackground='#466984',
                                  relief='flat', activestyle='none')
        self.listbox.pack(side='left', fill='both', expand=True)
        self.listbox.bind('<<ListboxSelect>>', self.select_quest)
        sb = tk.Scrollbar(body, command=self.listbox.yview); sb.pack(side='right', fill='y')
    ...
    def refresh_quest_list(self):
        ...
        completed = set(self.cfg.get('completed', []))
    ...
    def build_route(self):
        self.route_title = tk.Label(self.route_tab, text='', bg='#15171a', fg='white',
                                    font=('Segoe UI', 16, 'bold'))
        self.route_title.pack(anchor='w', padx=12, pady=(12, 3))
        self.route_info = tk.Label(self.route_tab, text='', bg='#15171a', fg='#bfc5cc',
                                   justify='left', anchor='w', wraplength=450)
        self.route_info.pack(fill='x', padx=12, pady=5)
        self.steps = tk.Listbox(self.route_tab, bg='#20242a', fg='white',
                                selectbackground='#3f6685', relief='flat', activestyle='none')
        self.steps.pack(fill='both', expand=True, padx=12, pady=8)
        self.steps.bind('<Double-Button-1>', lambda e: self.advance_step())
    ...
    def refresh_route(self):
        ...
        q = self.selected
        steps = q.get('steps') or [{'title': 'Quest data not mapped yet', 'destination': 'Quest guide',
                                    'notes': 'This quest is in the catalog; detailed route data will be added.'}]
        self.route_title.config(text=q.get('name', 'Quest'))
        prereq = ', '.join(q.get('prerequisites', [])) or 'None recorded'
        members = 'Members' if q.get('members', True) else 'Free-to-play'
        idx = min(self.step, len(steps) - 1)
        nxt = steps[idx]
        self.route_info.config(text=f"{q.get('difficulty','Unknown')} • {members} • {'Ironman-aware' if self.iron.get() else 'Standard'}\n"
    ...
    def advance_step(self):
        ...
        self.refresh_route()
        if steps and self.step >= len(steps):
            self.complete_quest(silent=True)

    def prev_step(self):
        self.step = max(0, self.step - 1); self.refresh_route()

    def complete_quest(self, silent=False):
        name = self.selected.get('name')
        if name and name not in self.cfg.setdefault('completed', []):
            self.cfg['completed'].append(name)
            self.persist(); self.refresh_quest_list(); self.update_dashboard()
        if not silent:
            messagebox.showinfo('Quest Navigator', f'{name} marked complete.')
    ...
    def recommend_quest(self):
        ...
        for q in self.quests:
            ...
            prereq = set(q.get('prerequisites', []))
            if prereq.issubset(completed):
                score = (0 if q.get('members', True) else -2, len(prereq), len(q.get('steps', [])))
        ...
        self.step = 0
        self.refresh_route()
        messagebox.showinfo('Recommended next quest', self.selected.get('name', 'Quest'))

    def update_dashboard(self):
        if not hasattr(self, 'progress_text'): return
        total = len(self.quests)
        done = len(set(self.cfg.get('completed', [])))
        mapped = sum(1 for q in self.quests if q.get('steps'))
        steps = self.selected.get('steps') or []
        idx = min(self.step, len(steps)-1) if steps else 0
        nxt = steps[idx].get('destination', 'Quest guide') if steps else 'Detailed route pending'
        self.progress_text.config(text=f'Quest catalog: {total}\nDetailed route data: {mapped}\nCompleted: {done}\n\n'
                                        f'Client: {"attached" if self.client else "not detected"}\nOverlay: {"ON" if self.overlay_enabled.get() else "OFF"}')
        self.next_text.config(text=f"CURRENT QUEST\n{self.selected.get('name','Quest')}\n\nNEXT DESTINATION\n{nxt}")

    # ---------- external overlay ----------
    def build_map(self):
        tk.Label(self.map_tab, text='External route overlay', bg='#15171a', fg='white',
                 font=('Segoe UI', 16, 'bold')).pack(anchor='w', padx=12, pady=(12, 4))
        tk.Label(self.map_tab, text='The overlay follows the official OSRS window. Route points are screen-calibrated and can be saved per quest.\n'
                                  'It does not click, type, inject, or control the game.', bg='#15171a', fg='#bfc5cc',
                 justify='left', wraplength=450).pack(anchor='w', padx=12, pady=4)
        ...
        self.overlay_toggle = tk.Checkbutton(self.map_tab, text='Show route overlay', variable=self.overlay_enabled,
                                             command=self.toggle_overlay, bg='#15171a', fg='white', selectcolor='#15171a',
                                             activebackground='#15171a', activeforeground='white')
        self.overlay_toggle.pack(anchor='w', padx=12)
        tk.Button(self.map_tab, text='Attach / refresh OSRS window', command=self.attach_overlay,
                  bg='#3f6685', fg='white', relief='flat').pack(anchor='w', padx=12, pady=8)
        tk.Button(self.map_tab, text='Add screen waypoint', command=self.add_screen_waypoint,
                  bg='#30353b', fg='white', relief='flat').pack(anchor='w', padx=12, pady=4)
        tk.Button(self.map_tab, text='Clear route points', command=self.clear_screen_waypoints,
                  bg='#30353b', fg='white', relief='flat').pack(anchor='w', padx=12, pady=4)
    ...
    def attach_overlay(self):
        ...
        if not self.client:
            ...
            if hasattr(self, 'client_status'):
                self.client_status.config(text='Could not find an OSRS window. Start the official client, then click Attach again.')
            return
        self.client_label.config(text='OSRS: attached', fg='#79d279')
        self.client_status.config(text=f'Attached: {self.client[1]}\nWindow: {self.client[2]},{self.client[3]} → {self.client[4]},{self.client[5]}')
        self.ensure_overlay()
    ...
    def ensure_overlay(self):
        ...
        self.overlay_canvas = tk.Canvas(self.overlay, bg='#010101', highlightthickness=0)
        self.overlay_canvas.pack(fill='both', expand=True)
        self.overlay.protocol('WM_DELETE_WINDOW', self.destroy_overlay)
        self.overlay.bind('<Button-1>', self.overlay_click)

    def position_overlay(self):
        if not self.client or not self.overlay: return
        _, _, l, t, r, b = self.client
        w, h = max(1, r-l), max(1, b-t)
        self.overlay.geometry(f'{w}x{h}+{l}+{t}')
        self.draw_overlay()

    def draw_overlay(self):
        if not self.overlay_canvas or not self.overlay_enabled.get():
            return
        self.overlay_canvas.delete('all')
        if not self.overlay_points:
            return
        pts = [(int(x), int(y)) for x, y in self.overlay_points]
        if len(pts) > 1:
            self.overlay_canvas.create_line(*[v for p in pts for v in p], fill='#ffd34e', width=4, smooth=True)
        ...
        if pts:
            x, y = pts[-1]
            self.overlay_canvas.create_text(x, max(12, y-18), text='NEXT', fill='white', font=('Segoe UI', 9, 'bold'))

    def overlay_click(self, event):
        if not self.overlay or not self.overlay_enabled.get(): return
        self.overlay_points.append((event.x, event.y))
        self.save_points_for_selected()
        self.draw_overlay()
        self.refresh_waypoint_label()

    def add_screen_waypoint(self):
        if not self.client:
            self.attach_overlay()
        if not self.client: return
        self.ensure_overlay()
        self.position_overlay()
        messagebox.showinfo('Add waypoint', 'Click a point in the OSRS window.\nThe click only records a screen position; it is not sent to OSRS.')

    def clear_screen_waypoints(self):
        self.overlay_points = []
        self.save_points_for_selected()
        self.draw_overlay(); self.refresh_waypoint_label()

    def save_points_for_selected(self):
        all_points = self.cfg.setdefault('route_points', {})
        all_points[self.selected.get('name', 'Unknown')] = self.overlay_points
        self.persist()
    ...
    def refresh_waypoint_label(self):
        if hasattr(self, 'wp_label'):
            self.wp_label.config(text=f'Calibrated points for {self.selected.get("name","Quest")}: {len(self.overlay_points)}')
    ...
    def build_settings(self):
        tk.Label(self.settings_tab, text='Settings', bg='#15171a', fg='white',
                 font=('Segoe UI', 16, 'bold')).pack(anchor='w', padx=12, pady=(12, 8))
        tk.Checkbutton(self.settings_tab, text='Automatically follow the OSRS window', variable=self.auto_attach,
                       command=self.persist, bg='#15171a', fg='white', selectcolor='#15171a',
                       activebackground='#15171a', activeforeground='white').pack(anchor='w', padx=12, pady=5)
        tk.Label(self.settings_tab, text='F8 = hide/show navigator\nF9 = toggle click-through\n\n'
                                  'The app is an external assistant: it does not inject into OSRS or automate gameplay.',
    ...
    def apply_geometry(self):
        self.root.geometry(f'500x680+{self.cfg.get("x",80)}+{self.cfg.get("y",80)}')

    def toggle_visible(self):
        self.root.withdraw() if self.root.state() != 'withdrawn' else self.root.deiconify()
    ...
    def persist(self):
        self.cfg['ironman'] = self.iron.get()
        self.cfg['auto_attach'] = self.auto_attach.get()
        self.cfg['overlay_enabled'] = self.overlay_enabled.get()
        self.cfg['x'], self.cfg['y'] = self.root.winfo_x(), self.root.winfo_y()
        save_json(CONFIG, self.cfg)

    def close(self):
        self.persist(); self.destroy_overlay(); self.root.destroy()
