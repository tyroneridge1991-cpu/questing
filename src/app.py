...
def find_osrs_windows():
    ...
    def callback(hwnd, _):
        ...
        low = title.lower()
        # RuneLite often uses titles such as "RuneLite - username" rather than
        # exactly "RuneLite". Match the title anywhere, plus common official-client titles.
        if ('runelite' in low or 'old school runescape' in low or
                low.startswith('jagex') or 'osrs' in low):
...
class Navigator:
    ...
    def build_ui(self):
        ...
        tk.Label(header, text='OSRS QUEST NAVIGATOR', bg='#20242a', fg='white', font=('Segoe UI', 11, 'bold')).pack(side='left', padx=12, pady=10)
        ...
        for tab, name in [(self.dashboard_tab, 'Dashboard'), (self.quest_tab, 'Quests'), (self.route_tab, 'Route'), (self.map_tab, 'Overlay'), (self.settings_tab, 'Settings')]:
    ...
    def build_dashboard(self):
        ...
        self.progress_text = tk.Label(self.dashboard_tab, text='', bg='#15171a', fg='#c8cdd3', justify='left', font=('Segoe UI', 10)); self.progress_text.pack(anchor='w', padx=14, pady=6)
        card = tk.Frame(self.dashboard_tab, bg='#20242a'); card.pack(fill='x', padx=14, pady=12)
        self.next_text = tk.Label(card, text='', bg='#20242a', fg='white', justify='left', anchor='w', font=('Segoe UI', 11)); self.next_text.pack(fill='x', padx=12, pady=12)
    ...
    def build_quests(self):
        ...
        self.search = tk.StringVar(); ent = tk.Entry(top, textvariable=self.search, bg='#282d33', fg='white', insertbackground='white', relief='flat'); ent.pack(fill='x', ipady=7); ent.bind('<KeyRelease>', lambda e: self.refresh_quest_list())
    ...
    def refresh_quest_list(self):
        ...
        for quest in sorted(self.quests, key=lambda x: x.get('name', '')):
            ...
            prefix = '✓ ' if name in completed else '  '; suffix = '  [F2P]' if not quest.get('members', True) else ''
    ...
    def build_route(self):
        self.route_title = tk.Label(self.route_tab, text='', bg='#15171a', fg='white', font=('Segoe UI', 16, 'bold')); self.route_title.pack(anchor='w', padx=12, pady=(12, 3))
        self.route_info = tk.Label(self.route_tab, text='', bg='#15171a', fg='#bfc5cc', justify='left', anchor='w', wraplength=470); self.route_info.pack(fill='x', padx=12, pady=5)
        self.steps = tk.Listbox(self.route_tab, bg='#20242a', fg='white', selectbackground='#3f6685', relief='flat', activestyle='none'); self.steps.pack(fill='both', expand=True, padx=12, pady=8); self.steps.bind('<Double-Button-1>', lambda e: self.advance_step())
        bar = tk.Frame(self.route_tab, bg='#15171a'); bar.pack(fill='x', padx=12, pady=8)
        tk.Button(bar, text='← Previous', command=self.prev_step, bg='#30353b', fg='white', relief='flat').pack(side='left'); tk.Button(bar, text='Complete step →', command=self.advance_step, bg='#3f6685', fg='white', relief='flat').pack(side='right'); tk.Button(bar, text='Mark quest complete', command=self.complete_quest, bg='#30353b', fg='white', relief='flat').pack(side='right', padx=6)
        self.refresh_route()
    ...
    def refresh_route(self):
        ...
        self.route_title.config(text=q.get('name', 'Quest')); prereq = ', '.join(q.get('prerequisites', [])) or 'None recorded'; idx = min(self.step, len(steps)-1); nxt = steps[idx]
        ...
        for i, s in enumerate(steps):
            mark = '✓' if i < self.step else ('→' if i == self.step else '○'); self.steps.insert('end', f"{mark}  {i+1}. {s.get('title','Objective')} — {s.get('destination','Unknown')}")
        self.steps.selection_set(idx); self.steps.see(idx); self.update_dashboard()
    ...
    def complete_quest(self,silent=False):
        ...
        if name and name not in self.cfg.setdefault('completed',[]): self.cfg['completed'].append(name); self.persist(); self.refresh_quest_list(); self.update_dashboard()
    ...
    def recommend_quest(self):
        ...
        for q in self.quests:
            ...
            if set(q.get('prerequisites',[])).issubset(completed):
                score=(0 if not q.get('members',True) else 1,0 if q.get('steps') else 1,len(q.get('prerequisites',[]))); candidates.append((score,q))
    ...
    def update_dashboard(self):
        ...
        total=len(self.quests); done=len(set(self.cfg.get('completed',[]))); mapped=sum(1 for q in self.quests if q.get('steps')); steps=self.selected.get('steps') or []; idx=min(self.step,len(steps)-1) if steps else 0; nxt=steps[idx].get('destination','Quest guide') if steps else 'Detailed route pending'
    ...
    def build_map(self):
        ...
        self.client_status=tk.Label(self.map_tab,text='',bg='#20242a',fg='#c8cdd3',justify='left',anchor='w'); self.client_status.pack(fill='x',padx=12,pady=10)
        tk.Checkbutton(self.map_tab,text='Show route overlay',variable=self.overlay_enabled,command=self.toggle_overlay,bg='#15171a',fg='white',selectcolor='#15171a',activebackground='#15171a',activeforeground='white').pack(anchor='w',padx=12)
        ...
        self.wp_label=tk.Label(self.map_tab,text='',bg='#15171a',fg='#bfc5cc',justify='left'); self.wp_label.pack(anchor='w',padx=12,pady=8); self.refresh_waypoint_label()

    def attach_overlay(self):
        self.client=find_osrs_window()
        if not self.client:
            self.client_label.config(text='OSRS: not found',fg='#df7b6e'); self.client_status.config(text='No RuneLite/OSRS window found. Make sure RuneLite is running and its main game window is visible.'); self.update_dashboard(); return
        self.client_label.config(text='OSRS: attached',fg='#79d279'); self.client_status.config(text=f'Attached to: {self.client[1]}\nWindow: {self.client[4]-self.client[2]} × {self.client[5]-self.client[3]}'); self.ensure_overlay(); self.position_overlay(); self.update_dashboard()
    ...
    def ensure_overlay(self):
        ...
        self.overlay_canvas=tk.Canvas(self.overlay,bg='#010101',highlightthickness=0); self.overlay_canvas.pack(fill='both',expand=True); self.make_clickthrough(self.overlay)

    def make_clickthrough(self,window):
        if os.name!='nt': return
        hwnd=window.winfo_id(); GWL_EXSTYLE=-20; WS_EX_LAYERED=0x80000; WS_EX_TRANSPARENT=0x20; style=user32.GetWindowLongW(hwnd,GWL_EXSTYLE)|WS_EX_LAYERED|WS_EX_TRANSPARENT; user32.SetWindowLongW(hwnd,GWL_EXSTYLE,style)
    ...
    def draw_overlay(self):
        ...
        if len(pts)>1: self.overlay_canvas.create_line(*[v for p in pts for v in p],fill='#ffd34e',width=4,smooth=True)
        for i,(x,y) in enumerate(pts,1): self.overlay_canvas.create_oval(x-7,y-7,x+7,y+7,outline='#ffd34e',width=3); self.overlay_canvas.create_text(x+14,y,text=str(i),fill='white',anchor='w',font=('Segoe UI',10,'bold'))
        if pts: x,y=pts[-1]; self.overlay_canvas.create_text(x,max(12,y-18),text='NEXT',fill='white',font=('Segoe UI',9,'bold'))

    def add_route_point(self):
        if not self.client: self.attach_overlay()
        if not self.client: return
        _,_,l,t,r,b=self.client; maxx,maxy=r-l,b-t
    ...
    def clear_screen_waypoints(self): self.overlay_points=[]; self.save_points_for_selected(); self.draw_overlay(); self.refresh_waypoint_label()
    def save_points_for_selected(self): self.cfg.setdefault('route_points',{})[self.selected.get('name','Unknown')]=[list(p) for p in self.overlay_points]; self.persist()
    def load_points_for_selected(self): self.overlay_points=[tuple(p) for p in self.cfg.get('route_points',{}).get(self.selected.get('name',''),[])];
    ...
    def tick(self):
        if self.auto_attach.get():
            ...
            if found:
                self.client=found; self.client_label.config(text='OSRS: attached',fg='#79d279'); self.ensure_overlay(); self.position_overlay(); self.client_status.config(text=f'Attached to: {found[1]}')
            else:
    ...
    def build_settings(self):
        ...
        tk.Checkbutton(self.settings_tab,text='Automatically follow the OSRS window',variable=self.auto_attach,command=self.persist,bg='#15171a',fg='white',selectcolor='#15171a',activebackground='#15171a',activeforeground='white').pack(anchor='w',padx=12,pady=5)
        tk.Label(self.settings_tab,text='F8 = hide/show navigator\nF9 = toggle click-through for the control window\n\nThe overlay is always click-through. It never clicks or controls OSRS.',bg='#15171a',fg='#bfc5cc',justify='left').pack(anchor='w',padx=12,pady=12)
    ...
    def toggle_clickthrough(self):
        ...
        self.persist()
    def persist(self): self.cfg['ironman']=self.iron.get(); self.cfg['auto_attach']=self.auto_attach.get(); self.cfg['overlay_enabled']=self.overlay_enabled.get(); self.cfg['x'],self.cfg['y']=self.root.winfo_x(),self.root.winfo_y(); save_json(CONFIG,self.cfg)
